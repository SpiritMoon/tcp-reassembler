...
{
    HASHTBL *hashtbl = mymalloc(sizeof(HASHTBL));
    hashtbl->nodes = mycalloc(size, sizeof(HASHNODE *));
    hashtbl->size = size;
...
{
    static HASHTBL *hashtbl = NULL;
    if (hashtbl == NULL)
        hashtbl = hashtbl_create(DEF_HASH_SIZE, def_hashfunc, NULL);
    else if (hashtbl->size == 0)
        hashtbl = hashtbl_create(DEF_HASH_SIZE, hashtbl->hashfunc, hashtbl->freefunc);
    return hashtbl;
}
...
{
    return hashtbl_remove_n(node, 1, get_hash_table()->freefunc);
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < num; i++) {
        char *key = mymalloc(1000);
        char *data = mymalloc(1000);
        sprintf(key, "%d", i);
